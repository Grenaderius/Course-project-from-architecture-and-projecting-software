...
import Actions.AgencyActions.CountOfRepeadsFinder;
...
public class HotelsListAndBookedFlatsCommand implements Command, FileChooserInterface {
    ...
    public void execute() {
        if(checkFileType()){
            ColumnFinder columnFinder = new ColumnFinder(chosenFile, typeForDividerChooser.getTypeForHotels());
            CountOfRepeadsFinder countOfRepeadsFinder = new CountOfRepeadsFinder(columnFinder.findNum(tablesFillerPatterns.returnGroupPattern().split("\\|")) + 1, columnFinder.getList());
            System.out.println("Список готелів та зайнятих у них номерів:\n" + countOfRepeadsFinder.getColumnWithNumberOfReplacedElements());
